//! `#[wgpui_kit::test]` is GPUI's test attribute reached through the umbrella
//! crate; it needs the `test-support` feature.
//!
//! Test modules import what they use rather than `use wgpui_kit::*;`: with
...
use wgpui_kit::{AppContext as _, TestAppContext};
...
fn ordinary_rust_test_keeps_the_builtin_attribute() {
    assert_eq!(
        wgpui_kit::size(wgpui_kit::px(12.), wgpui_kit::px(24.)).width,
        wgpui_kit::px(12.)
    );
}
