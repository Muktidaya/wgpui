use wgpui_kit::test::{TestAppContextExt, TestSupportExt, TestWindowExt};
use wgpui_kit::{
...
struct ChoiceStates {
    checkbox: wgpui_kit::base::CheckboxState,
    radio: bool,
    pressed: bool,
}
impl Render for ChoiceStates {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .gap_4()
            .child(
                wgpui_kit::base::Checkbox::new("mixed")
                    .state(self.checkbox)
                    .size(px(40.))
            ...
            .child(
                wgpui_kit::base::Radio::new("radio")
                    .checked(self.radio)
                    .size(px(40.))
            ...
            .child(
                wgpui_kit::base::Toggle::new("toggle")
...
fn mixed_checkbox_radio_and_toggle_report_distinct_states(cx: &mut TestAppContext) {
    cx.update(wgpui_kit::init);
    let handle = cx.add_window(|_, _| ChoiceStates {
        checkbox: wgpui_kit::base::CheckboxState::Indeterminate,
        radio: false,
        pressed: false,
    });
    cx.update_window(handle.into(), |_, window, cx| {
...
struct VirtualRows {
    scroll: wgpui_kit::base::VirtualListScrollHandle,
}
...
fn scrolling_a_real_virtual_list_registers_new_rows_and_releases_old_ones(cx: &mut TestAppContext) {
    let handle = cx.open_window(size(px(200.), px(200.)), |_, _| VirtualRows {
        scroll: wgpui_kit::base::VirtualListScrollHandle::new(),
    });
    cx.update_window(handle.into(), |_, window, cx| {
        window.render_frame(cx);
...
struct Pair<T: Render> {
    left: wgpui_kit::Entity<T>,
    right: wgpui_kit::Entity<T>,
}
...
struct KeyboardJump {
    left: wgpui_kit::FocusHandle,
    right: wgpui_kit::FocusHandle,
