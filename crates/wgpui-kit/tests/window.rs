...
impl Render for FocusParts {
    fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl IntoElement {
        div()
            ...
            .flex_col()
            .child(
                wgpui_kit::base::AccordionTrigger::new("disclosure")
                    .track_focus(&self.handles[0])
                    .size_8(),
            )
            .child(
                wgpui_kit::base::TableRow::new("row", 1)
                    .track_focus(&self.handles[1])
                    .size_8(),
            )
            .child(
                wgpui_kit::base::TableCell::new("cell", 1)
                    .track_focus(&self.handles[2])
                    .size_8(),
            )
            .child(
                wgpui_kit::base::TableHeader::new("header")
