...
use wgpui_kit::test::{TestAppContextExt, TestWindowExt};
...
impl Render for HoverHelp {
    fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl IntoElement {
        use wgpui_kit::component::{button::Button, hover_card::HoverCard};
        use wgpui_kit::test::TestSupportExt as _;
...
async fn real_hover_card_opens_and_closes_after_pointer_delays(cx: &mut TestAppContext) {
    ...
    let handle = cx.open_window(size(px(640.), px(480.)), |window, cx| {
        ...
        wgpui_kit::component::Root::new(view, window, cx)
    });
    cx.update_window(handle.into(), |_, window, cx| {
        window.render_frame(cx);
