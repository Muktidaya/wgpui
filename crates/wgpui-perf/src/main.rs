...
//!
//! # Setup
//! Hyperfine is optional; without it the runner times ten subprocess samples.
//!
//! # Usage
...
//! `cargo perf-test -p wgpui -- --json=blah` will result in `.perf-runs/blah.wgpui.json`
    ...
    fn log(&self, output: &Output, t_bin: &str) {
        match self {
            ...
            OutputKind::Json(ident) => {
                // We're going to be in crates/wgpui-perf/$whatever.
                let wspace_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                    .join("..")
                    .join("..");
                ...
                // target/release-fast/deps/wgpui-061ff76c9b7af5d7
                // would be reduced to just "wgpui".
...
fn compare_profiles(args: &[String]) {
    ...
    let wspace_dir = env!("CARGO_MANIFEST_DIR");
...
fn hyp_binary() -> Option<Command> {
    ...
    let home_binary = cargo_home.map(|home| home.join("bin/hyperfine"));
    if Command::new(HYP_PATH).arg("--version").output().is_ok() {
        return Some(Command::new(HYP_PATH));
    }
    let binary = home_binary?;
    Command::new(&binary).arg("--version").output().ok()?;
    Some(Command::new(binary))
}

/// Profiles a test with hyperfine or subprocess samples, returning mean and standard deviation
...
fn hyp_profile(t_bin: &str, t_name: &str, iterations: NonZero<usize>) -> Option<Timings> {
    let Some(mut perf_cmd) = hyp_binary() else {
        // Keep the runner usable without an optional external measurement tool.
        // Match its subprocess timing model, including one discarded warmup.
        const SAMPLE_COUNT: u32 = 10;
        spawn_and_iterate(t_bin, t_name, iterations)?;
        let samples = (0..SAMPLE_COUNT)
            .map(|_| {
                spawn_and_iterate(t_bin, t_name, iterations).map(|duration| duration.as_secs_f64())
            })
