...
pub(crate) fn derive_action(input: TokenStream) -> TokenStream {
    ...
    */
    ...
    let build_fn_body = if no_json {
        ...
        quote! { Err(wgpui::private::anyhow::anyhow!(#error_msg)) }
    } else if is_unit_struct {
        quote! { Ok(Box::new(Self)) }
    } else {
        quote! { Ok(Box::new(wgpui::private::serde_json::from_value::<Self>(_value)?)) }
    };

    let json_schema_fn_body = if no_json || is_unit_struct {
        quote! { None }
    } else {
        quote! { Some(<Self as wgpui::private::schemars::JsonSchema>::json_schema(_generator)) }
    };
    ...
    TokenStream::from(quote! {
        ...
        impl wgpui::Action for #struct_name {
            ...
            fn partial_eq(&self, action: &dyn wgpui::Action) -> bool {
                action
                    .as_any()
                    .downcast_ref::<Self>()
            ...
            fn build(_value: wgpui::private::serde_json::Value) -> wgpui::Result<Box<dyn wgpui::Action>> {
                #build_fn_body
            }

            fn action_json_schema(
                _generator: &mut wgpui::private::schemars::SchemaGenerator,
            ) -> Option<wgpui::private::schemars::Schema> {
                #json_schema_fn_body
            }
